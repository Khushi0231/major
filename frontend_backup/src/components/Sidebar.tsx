...
export default function Sidebar({
  ...
  return (
    <div className="flex flex-col h-full bg-[#0c111f] text-gray-200">
      ...
      {/* Navigation */}
      <div className="px-3 py-4 space-y-2 border-b border-gray-900">
        {menuItems.map((item) => (
          <button
            key={item.id}
            onClick={() => setActive(item.id as any)}
            className={`w-full flex items-center gap-3 px-3 py-2 rounded-xl transition-all ${
              active === item.id ? "bg-blue-600/20 text-white border border-blue-500/40" : "text-gray-400 hover:bg-gray-900"
            }`}
          >
        ...
        {sessions.map((session) => (
          ...
          >
            ...
            <div className="text-[11px] text-gray-500">
              Offline • {new Date(session.timestamp).toLocaleTimeString()}
            </div>
          </button>
        ))}
        {sessions.length === 0 && (
